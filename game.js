...
log("Your turn — 5 AP. Bank starts at 0; +2/turn from turn 2 (cap 4). Shortfall auto-pulls from bank.","sys");
...
// First player turn of the fight: bank stays 0 (no +2 refresh). Refresh from turn 2 onward.
if(!first){
...
const fromAp=Math.min(state.ap,n);
state.ap-=fromAp;
const fromBank=n-fromAp;
if(fromBank>0){state.bank-=fromBank;log("Spent "+fromAp+" AP + "+fromBank+" bank (AP "+state.ap+"/5, bank "+state.bank+"/4).","sys");}
